...
export const MessageList = (props: any) => {
  ...
  useEffect(() => {
    if (userSelector.hashId) getChats();
  }, [userSelector.hashId]);
      ...
      .then(response => {
        if (response.data.success) {
          ...
          if (cs && cs.length > 0) {
            ...
            if (cs[0].users && cs[0].users.userFrom && cs[0].users.userFrom.userId === userSelector.hashId) {
              otherUserId = cs[0].users.userTo.userId;
            } else if (
              cs[0].users &&
              cs[0].users.userTo &&
              cs[0].users.userTo.userId === userSelector.hashId
            ) {
          ...
          {chats && chats.length > 0 ? (
            chats
              .filter(chat => {
                if (keyword.length > 0) {
                  const name =
                    userSelector.hashId === chat.users.userTo.userId
