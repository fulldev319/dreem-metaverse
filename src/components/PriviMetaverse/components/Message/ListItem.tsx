...
export const ListItem: React.FunctionComponent<any> = ({
  ...
  useEffect(() => {
    if (chat) {
      if (chat.users && chat.users.userFrom && chat.users.userFrom.userId === userSelector.hashId) {
        setOtherUser(chat.users.userTo);
      } else if (chat.users && chat.users.userTo && chat.users.userTo.userId === userSelector.hashId) {
