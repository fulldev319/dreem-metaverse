...
import { Grid, useMediaQuery, useTheme, Select, MenuItem } from "@material-ui/core";
...
const STATUS_OPTIONS = ["All", "Draft", "Minted"];
const ASEET_TYPES = ["All", "Draft", "Minted"];

export default function AssetsPage() {
  const classes = usePageStyles();

  const filterClasses = useFilterSelectStyles();
  const [selAssetsMenu, setSelAssetsMenu] = useState("assets");
  ...
  const [filterStatus, setFilterStatus] = React.useState<string>(STATUS_OPTIONS[0]);
  const [filterAssetType, setFilterAssetType] = React.useState<string>(ASEET_TYPES[0]);
  ...
  const loadAssets = () => {
    setIsLoading(true);
    MetaverseAPI.getAssets(12,curPage,"DESC",["MODEL"], undefined, undefined, undefined, undefined, false, false, undefined)
      .then(res => {
  ...
  return (
    <>
      <Box display="flex" alignItems="center" justifyContent="space-between" mt={5.5}>
        ...
        >
          ...
          <Select
            ...
            onChange={(e: any) => setFilterAssetType(e.target.value)}
            className={classes.select}
            renderValue={(value: any) => (
            ...
            IconComponent={ArrowIconComponent(setOpenAssetTypeSelect)}
          >
            {ASEET_TYPES.map((status, index) => (
              <MenuItem key={`filter-status-${index}`} value={status}>
                {status}
              </MenuItem>
            ))}
          ...
          loader={
            isLoading && (<Box mt={2}>
              <MasonryGrid
                gutter={"16px"}
                data={Array(3).fill(0)}
                renderItem={(item, _) => <AssetCard item={{}} isLoading={true} />}
        ...
        >
          <Grid container spacing={3} style={{ marginBottom: 24 }}>
            {assets?.map((item, index) => (
                <Grid item key={`asset-card-${index}`} md={4} sm={6} xs={12}>
                  <AssetCard item={item} />
                </Grid>
              ))}
          </Grid>
...
const GlobalIcon = () => (
  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M14 0.4375C6.50962 0.4375 0.4375 6.50962 0.4375 14C0.4375 21.4904 6.50962 27.5625 14 27.5625C21.4904 27.5625 27.5625 21.4904 27.5625 14C27.5625 6.50962 21.4904 0.4375 14 0.4375ZM2.375 14C2.375 13.5796 2.40019 13.163 2.44475 12.7542L10.125 17.875V19.8125C10.125 20.882 10.993 21.75 12.0625 21.75V25.4506C6.57356 24.5226 2.375 19.7486 2.375 14ZM21.75 22.6393V21.75C21.75 20.6805 20.882 19.8125 19.8125 19.8125H17.875V15.9375C17.875 14.868 17.007 14 15.9375 14H8.1875V12.0625H10.125C11.1945 12.0625 12.0625 11.1945 12.0625 10.125V8.1875H15.9375C17.007 8.1875 17.875 7.3195 17.875 6.25V3.05312C22.3836 4.6535 25.625 8.95088 25.625 14C25.625 17.4313 24.1215 20.51 21.75 22.6393Z"
      fill="white"
    />
  </svg>
);
