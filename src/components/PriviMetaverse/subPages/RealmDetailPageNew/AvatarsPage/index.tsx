...
export default function AvatarsPage() {
  ...
  const [topAvatars, setTopAvatars] = React.useState<any>([]);
  const [avatars, setAvatars] = React.useState<any>([]);
  const [curPage, setCurPage] = React.useState<number>(1);
  const [hasMore, setHasMore] = React.useState<boolean>(true);

  const loadTopAvatars = () => {
    setIsLoading(true);
    MetaverseAPI.getAssets(12,1,"DESC",["CHARACTER"], undefined, undefined, undefined, undefined, false, false, undefined)
      .then(res => {
        setTopAvatars(res.data.elements);
      })
      .finally(() => setIsLoading(false));
  };
  const loadAvatars = () => {
    setIsLoading(true);
    MetaverseAPI.getAssets(12,curPage,"DESC",["CHARACTER"], undefined, undefined, undefined, undefined, false, false, undefined)
      .then(res => {
        setAvatars(res.data.elements);
  ...
  return (
    <>
      <Box display="flex" alignItems="flex-start" justifyContent="space-between" mt={5.5}>
        <Box>
          ...
          <Box mt={3} className={classes.typo8}>
            These avatars allow you to enter this realm [Realm name]
          </Box>
        </Box>
          ...
          loader={
            <Box mt={2}>
              <MasonryGrid
                gutter={"16px"}
                data={Array(4).fill(0)}
                renderItem={(item, _) => <AvatarCard item={{}} isLoading={true} />}
        ...
        >
          {topAvatars && <Box mt={2}>
            <MasonryGrid
              gutter={"16px"}
              data={topAvatars}
              renderItem={(item, index) => <AvatarCard item={item} />}
              columnsCountBreakPoints={COLUMNS_COUNT_BREAK_POINTS_FOUR}
            />
          </Box>
          }
