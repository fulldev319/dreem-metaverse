...
export default function VotingPage() {
  ...
  return (
    <>
      <Box display={"flex"} alignItems={"center"} justifyContent={"space-between"} mt={7} mb={3}>
        ...
        <Box display={"flex"} alignItems={"center"}>
          <Box mr={2.5} className={classes.typo8}>
            Status
          </Box>
          <Box>
            <Select
      ...
      >
        <Grid container style={{ marginBottom: 24 }}>
          {Array(9).fill(0).map((item, index) => (
            <Grid md={3} sm={6} xs={12} style={{padding: 10}}>
              <OwnedAssetCard key={`owned-asset-${index}`}/>
            </Grid>
          ))}
        </Grid>
      </InfiniteScroll>
